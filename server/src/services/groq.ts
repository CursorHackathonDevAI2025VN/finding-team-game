...
${
  isLeaderLooking
    ? "A team leader is looking for members"
    : "A member is looking for teams/leaders"
}.
    ...
    .map((candidate) => {
      ...
      candidateSkillsLower.forEach((skill) => {
        if (
          ...
          matchedSkills.push(
            candidate.skills[candidateSkillsLower.indexOf(skill)]
          );
        }
      });
      ...
      return {
        ...
        matchedSkills,
        reason: `Position: ${
          candidate.position === position ? "Match" : "Different"
